...
struct arch *arch)
{
	...
	for(i = 0; i < arch->object->mh64->ncmds; i++){
		if(lc->cmd == LC_SEGMENT_64){
...
#ifndef HACK_TO_MATCH_TEST_CASE
			if(strcmp(sg64->segname, SEG_TEXT) == 0){
				strcpy(scnhdrs[scnhdrsIdx].s_name, ".text");
				scnhdrs[scnhdrsIdx].s_vsize = sg64->vmsize;
				scnhdrs[scnhdrsIdx].s_vaddr = sg64->vmaddr;
				scnhdrs[scnhdrsIdx].s_size = rnd(sg64->filesize, file_alignment);
				scnhdrs[scnhdrsIdx].s_relptr = 0;
				scnhdrs[scnhdrsIdx].s_lnnoptr = 0;
				scnhdrs[scnhdrsIdx].s_nlnno = 0;
				scnhdrs[scnhdrsIdx].s_flags = IMAGE_SCN_MEM_EXECUTE |
				IMAGE_SCN_MEM_READ |
				IMAGE_SCN_CNT_CODE;
				scn_contents[scnhdrsIdx] = object_addr + sg64->fileoff;
				scnhdrsIdx++;
				j++;
			}
			else if(strcmp(sg64->segname, SEG_DATA) == 0){
				...
				{
					if ( strcmp(s64->sectname, "__mod_init_func") == 0 ) {
						snprintf(scnhdrs[scnhdrsIdx].s_name, sizeof(scnhdrs[scnhdrsIdx].s_name), ".ctorss");
						scnhdrs[scnhdrsIdx].s_vsize = s64->size;
						scnhdrs[scnhdrsIdx].s_vaddr = s64->addr;
						scnhdrs[scnhdrsIdx].s_size = rnd(s64->size, file_alignment);
						scnhdrs[scnhdrsIdx].s_relptr = 0;
						scnhdrs[scnhdrsIdx].s_lnnoptr = 0;
						scnhdrs[scnhdrsIdx].s_nlnno = 0;
						scnhdrs[scnhdrsIdx].s_flags = IMAGE_SCN_MEM_READ |
						IMAGE_SCN_MEM_WRITE |
						IMAGE_SCN_CNT_CODE |
						IMAGE_SCN_CNT_INITIALIZED_DATA |
						IMAGE_SCN_MEM_EXECUTE;
						scn_contents[scnhdrsIdx] = object_addr + s64->offset;
						scnhdrsIdx++;
						j++;
					}
				}
				
				strcpy(scnhdrs[scnhdrsIdx].s_name, ".data");
				scnhdrs[scnhdrsIdx].s_vsize = sg64->vmsize;
				scnhdrs[scnhdrsIdx].s_vaddr = sg64->vmaddr;
				scnhdrs[scnhdrsIdx].s_size = rnd(sg64->filesize, file_alignment);
				scnhdrs[scnhdrsIdx].s_relptr = 0;
			...
			((char *)sg64 + sizeof(struct segment_command_64));
			for(i = 0; i < sg64->nsects; i++, s64++){
				sprintf(scnhdrs[scnhdrsIdx].s_name, "/%d", strsize);
	...
	if(reloc_size != 0){
	    strcpy(scnhdrs[scnhdrsIdx].s_name, ".reloc");
	    scnhdrs[scnhdrsIdx].s_vsize = reloc_size;
	    reloc_addr = rnd(reloc_addr, section_alignment);
	    scnhdrs[scnhdrsIdx].s_vaddr = reloc_addr;
	    scnhdrs[scnhdrsIdx].s_size = rnd(reloc_size, file_alignment);
	    scnhdrs[scnhdrsIdx].s_relptr = 0;
	    scnhdrs[scnhdrsIdx].s_lnnoptr = 0;
	    scnhdrs[scnhdrsIdx].s_nlnno = 0;
	    scnhdrs[scnhdrsIdx].s_flags = IMAGE_SCN_MEM_READ |
				 ...
				 IMAGE_SCN_MEM_EXECUTE;
	    reloc_scnhdr = scnhdrs + j;
	    scn_contents[scnhdrsIdx] = reloc_contents;
		scnhdrsIdx++;
	    j++;
	    debug_addr = reloc_addr + reloc_scnhdr->s_size;
	}
	...
	if(debug_filename != NULL){
	    strcpy(scnhdrs[scnhdrsIdx].s_name, ".debug");
	    scnhdrs[scnhdrsIdx].s_vsize = debug_size;
	    scnhdrs[scnhdrsIdx].s_vaddr = debug_addr;
	    scnhdrs[scnhdrsIdx].s_size = rnd(debug_size, file_alignment);
	    scnhdrs[scnhdrsIdx].s_relptr = 0;
	    scnhdrs[scnhdrsIdx].s_lnnoptr = 0;
	    scnhdrs[scnhdrsIdx].s_nlnno = 0;
	    scnhdrs[scnhdrsIdx].s_flags = IMAGE_SCN_MEM_READ |
				 ...
				 IMAGE_SCN_MEM_EXECUTE;
	    debug_scnhdr = scnhdrs + j;
	    scn_contents[scnhdrsIdx] = debug_contents;
	    j++;
		scnhdrsIdx++;
	}
